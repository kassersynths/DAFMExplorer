/**
 * Offline YM2612 note renderer for CON-EDA exemplars (AIFF for Adobe inline PDF sound).
 * Usage (from repo root): npx --yes tsx scripts/render_con_eda_aiffs.ts
 */
...
const SUMMARY = path.join(ROOT, "artifacts", "con_eda_summary.json");
const OUT_DIR = path.join(ROOT, "Ludo2026-DAFMExplorer-LaTeX", "videos", "presets");

const CLOCK = 7670448;
const SAMPLE_RATE = 44100;
const NOTE_MIDI = 60; // C4 — fixed for all exemplars
const ATTACK_S = 0.05;
const HOLD_S = 1.2;
...
function writePatch(ym: any, patch: Exemplar["params"]) {
  const ch = 0;
  const valB0 = ((patch.feedback & 7) << 3) | (patch.algorithm & 7);
  ym.write(0xb0 + ch, valB0);
  const valB4 = 0xc0 | ((patch.ams & 3) << 4) | (patch.pms & 7);
  ym.write(0xb4 + ch, valB4);
...
/** Apple 80-bit extended float for AIFF COMM sample rate. */
...
/** AIFF: 16-bit PCM big-endian mono (Adobe / Beamer \\sound). */
function encodeAiff(mono: Float32Array, sampleRate: number): Buffer {
  ...
  let ssndPayload = Buffer.concat([Buffer.alloc(8), ssndData]); // offset + blockSize
  if (ssndPayload.length % 2) ssndPayload = Buffer.concat([ssndPayload, Buffer.alloc(1)]);

  const comm = Buffer.concat([
    Buffer.from([0, 1]), // channels
    (() => {
      const b = Buffer.alloc(4);
      b.writeUInt32BE(n, 0);
      return b;
    })(),
    Buffer.from([0, 16]), // sample size
    float80(sampleRate),
  ]);
  let commPad = comm;
...
function audioName(ex: Exemplar): string {
  if (ex.aiff) return ex.aiff;
  if (ex.wav) return ex.wav.replace(/\.wav$/i, ".aiff");
  return `star${ex.star_id}_ex${ex.exemplar}.aiff`;
}

function main() {
  if (!fs.existsSync(SUMMARY)) {
    console.error("Missing", SUMMARY, "— run scripts/con_conditioned_eda.py first");
    process.exit(1);
  }
  fs.mkdirSync(OUT_DIR, { recursive: true });
  const summary = JSON.parse(fs.readFileSync(SUMMARY, "utf8"));
  const exemplars: Exemplar[] = summary.exemplars;
  const manifest = {
    format: "aiff",
    note_midi: NOTE_MIDI,
  ...
  for (const ex of exemplars) {
    const name = audioName(ex);
    console.log("Rendering", name, `(CON=${ex.CON} ${ex.Game})`);
    const pcm = renderExemplar(ex);
    fs.writeFileSync(path.join(OUT_DIR, name), encodeAiff(pcm, SAMPLE_RATE));
    manifest.files.push({
      aiff: name,
      star_id: ex.star_id,
      ...
      Game: ex.Game,
      Name: ex.Name,
      share_within_con: ex.share_within_con,
      narrative: ex.narrative,
    });
  }

  fs.writeFileSync(path.join(OUT_DIR, "manifest.json"), JSON.stringify(manifest, null, 2));
  console.log("Wrote", exemplars.length, "AIFFs + manifest.json →", OUT_DIR);
}
